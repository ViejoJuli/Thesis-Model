...
def main():
    ...
    @st.cache_resource
    def init_connection():
        uri = "mongodb+srv://jdrios13:Mifirafa1@cluster0.xbdlmri.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    ...
    def get_data():
        db = client['app-data']
        items = db.users.find()
        print(f"first check {items}")
        print(list(items))
        # items = list(items)  # make hashable for st.cache_data
        return  # items
    items = get_data()
    # print(f"this are the items {items}")
    ...
    if query := st.chat_input("Cual es tu consulta"):
        ...
        print(f"these are accumulated: {st.session_state['costs']}")
